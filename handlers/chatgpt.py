...
async def lessons_msg(msg: Message) -> None:
    ...
    try:
        ...
        if msg.text == '/chatgpt':
            ...
            # TODO answer_msg = await msg.answer('ChatGPT думает...')
            send_text = ask_gpt(
                ' '.join(msg.text.split()[1:]),
                msg.from_user.first_name,
            )
            # TODO await msg.edit_text(send_text)

            # ВРЕМЕННОЕ РЕШЕНИЕ >>>
            await msg.answer(send_text)

    except Exception as e:
        await answer_msg.edit_text(error(e, msg.from_user.language_code), parse_mode='HTML')
